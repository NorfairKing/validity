...
succeedsOnGen ::
     (Show a, Show b, Show (f b), CanFail f) => (a -> f b) -> Gen a -> (a -> [a]) -> Property
...
succeedsOnArbitrary ::
     (Show a, Show b, Show (f b), Arbitrary a, CanFail f) => (a -> f b) -> Property
...
succeeds2 ::
     (Show a, Show b, Show c, Show (f c), GenUnchecked a, GenUnchecked b, CanFail f)
  => (a -> b -> f c)
  -> Property
