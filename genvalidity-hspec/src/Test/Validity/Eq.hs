    ...
    parallel $ do
        ...
        describe ("Eq " ++ name) $ do
            ...
            describe funeqstr $ do
                it "is reflexive" $ reflexivityOnGen eq gen
                it "is symmetric" $ symmetryOnGens eq gen2
                it "is transitive" $ transitivityOnGens eq gen3
                it "is equivalent to (\\a b -> not $ a /= b)" $
                    equivalentOnGens2 eq (\a b -> not $ a `neq` b) gen2
            describe funneqstr $ do
                it "is antireflexive" $ antireflexivityOnGen neq gen
                it "is equivalent to (\\a b -> not $ a == b)" $
...
eqSpecOnValid
    ...
    => Proxy a -> Spec
eqSpecOnValid proxy = eqSpecOnGen proxy genValid
