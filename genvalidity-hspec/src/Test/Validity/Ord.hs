    ...
    parallel $ do
        ...
        describe ("Ord " ++ name) $ do
            describe funlestr $ do
                it "is reflexive" $ reflexivityOnGen cmple gen
                it "is antisymmetric" $ antisymmetryOnGens cmple gen2
                it "is transitive" $ transitivityOnGens cmple gen3
                it "is equivalent to (\\a b -> compare a b /= GT)" $
                    equivalentOnGens2 cmple (\a b -> compare a b /= GT) gen2
            describe fungestr $ do
                it "is reflexive" $ reflexivityOnGen cmpge gen
                it "is antisymmetric" $ antisymmetryOnGens cmpge gen2
                it "is transitive" $ transitivityOnGens cmpge gen3
                it "is equivalent to (\\a b -> compare a b /= LT)" $
                    equivalentOnGens2 cmpge (\a b -> compare a b /= LT) gen2
            describe funltstr $ do
                it "is antireflexive" $ antireflexivityOnGen cmplt gen
                it "is transitive" $ transitivityOnGens cmplt gen3
                it "is equivalent to (\\a b -> compare a b == LT)" $
                    equivalentOnGens2 cmplt (\a b -> compare a b == LT) gen2
            describe fungtstr $ do
                it "is antireflexive" $ antireflexivityOnGen cmpgt gen
                it "is transitive" $ transitivityOnGens cmpgt gen3
                it "is equivalent to (\\a b -> compare a b == GT)" $
...
ordSpecOnValid
    ...
    => Proxy a -> Spec
ordSpecOnValid proxy = ordSpecOnGen proxy arbitrary
